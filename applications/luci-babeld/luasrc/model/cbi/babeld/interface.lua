...
m = Map("babeld", translatef("Babeld - Interface %q", "?"),
...
if m.uci:get("babeld", sid) ~= "interface" then
	luci.http.redirect(m.redirect)
	return
end

m.uci:foreach("babeld", "interface",
	function(s)
		if s['.name'] == sid and s.interface then
			m.title = translatef("Babeld - Interface %q", s.interface)
			return false
		end
	end)
...
function o.write(self, section, value)
	if value == "1" then
		m.uci:set("babeld", section, "ignore", 0)
		m.uci:set("babeld", section, "IgnoreIfMissing", 1)
	end

	m.uci:set("babeld", section, "AdvSendAdvert", value)
end
